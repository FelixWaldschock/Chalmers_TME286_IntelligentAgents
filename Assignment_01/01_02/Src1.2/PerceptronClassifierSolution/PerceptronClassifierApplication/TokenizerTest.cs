    ...
    static void Main()
    {
        ...
        foreach (string token in tokens)
        {
            if (!string.IsNullOrWhiteSpace(token)) // This check removes empty entries from the result
            {
                ...
                // make all lowercase
                finalTokens.Add(token.ToLower());
            }
        }
        
        // now handle the '.' character
        
        List<string> abbrevations = new List<string> { "mr.", "mrs", "e.g.", "i.e." };

        foreach (string token in finalTokens.ToList()) // Use ToList() to avoid modifying the collection during iteration
        {
            // check if token is an abbreviation
            if (abbrevations.Contains(token))
            {
                // do nothing
            }
